...
const base = {
    ...
    locale: Joi.string().optional().default('nb-NO'),
    cdnHost: Joi.string().optional().allow('').default(''),
    domain: Joi.string().default('finn.no'),
    baseUrl: Joi.string().allow('').default(''),
    resourceMountPath: Joi.string().default('/podium-resources'),
...
const requestContextSchema = Joi.object().keys(base).unknown(false);

const contextSchema = Joi.object()
    .keys(
        Object.assign({}, base, {
            token: Joi.string().optional(), // jwt-token?
            userId: Joi.string().optional(),
            spidId: Joi.string().optional(),
...
const validateProm = (schema, context) =>
    new Promise((resolve, reject) => {
        const result = schema.validate(context, { abortEarly: false });
        if (result.error) {
            reject(result.error);
        } else {
